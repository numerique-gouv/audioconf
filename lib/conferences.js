...
const OVHRoomApiHelper = {
  ...
  removeRoomPinAndSetExpirationDate: async (roomNumber, durationInMinutes) => {
    const now = new Date()
...
const conferences = {
  createConfWithNumbersApi: async (durationInMinutes) => {
  ...
  createConfWithRoomsApi: async (durationInMinutes) => {
    try {
      const roomNumber = await OVHRoomApiHelper.createRoom()
      const freeAt = await OVHRoomApiHelper.removeRoomPinAndSetExpirationDate(roomNumber, durationInMinutes)
      return {
...
module.exports.createConf = async (durationInMinutes) => {
  if (config.USE_OVH_ROOM_API) {
    return conferences.createConfWithRoomsApi(durationInMinutes)
  }
  return conferences.createConfWithNumbersApi(durationInMinutes)
}
