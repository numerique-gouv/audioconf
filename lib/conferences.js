...
module.exports.addPublicWebAccessToRoom = async (phoneNumber) => {
  ...
  try {
    ...
    console.error(`Error addPublicWebAccessToRoom on ${url}`, err)
    throw new Error(`Error addPublicWebAccessToRoom on ${url} : ${JSON.stringify(err)}`)
  }
}
