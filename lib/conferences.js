...
module.exports.lockRoom = async (phoneNumber, roomNumber) => {
  ...
  try {
    ...
    console.error(`Error lockRoom on ${url}`, err)
    throw new Error(`Error lockRoom on ${url} : ${JSON.stringify(err)}`)
  }
}
...
module.exports.unlockRoom = async (phoneNumber, roomNumber) => {
  ...
  try {
    ...
    console.error(`Error unlockRoom on ${url}`, err)
    throw new Error(`Error unlockRoom on ${url} : ${JSON.stringify(err)}`)
  }
}


module.exports.fetchDashboardInfo = async (phoneNumber, roomNumber) => {
  const url = `/telephony/${config.OVH_ROOM_ACCOUNT_NUMBER}/conference/${phoneNumber}/rooms/${roomNumber}/participants`
  try {
    return await ovhRooms.requestPromised("GET", url)
  } catch (err) {
    console.error(`Error fetchDashboardInfo on ${url}`, err)
    throw new Error(`Error fetchDashboardInfo on ${url} : ${JSON.stringify(err)}`)
  }
}
...
module.exports.getParticipant = async (phoneNumber, roomNumber, participantId) => {
  ...
  try {
    ...
    console.error(`Error getParticipant on ${url}`, err)
    throw new Error(`Error getParticipant on ${url} : ${JSON.stringify(err)}`)
  }
}
