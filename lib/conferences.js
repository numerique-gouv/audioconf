...
const OVHRoomApiHelper = {
  ...
  getAllRooms: async () => {
    ...
    try {
      ...
      console.error(`OVH Error getAllRooms on ${url}`, err)
      throw new Error(`OVH Error getAllRooms on ${url} : ${JSON.stringify(err)}`);
    }
  },
  getNumberOnlineParticipantsInRoom: async (roomNumber) => {
    const url = `/telephony/${config.OVH_ROOM_ACCOUNT_NUMBER}/conference/${config.OVH_ROOM_PHONE_NUMBER}/rooms/${roomNumber}/participants`
    try {
      const participants = await ovhRooms.requestPromised(
        'GET', url
      )
      return participants.length
    } catch (err) {
      console.error(`OVH Error getNumberOnlineParticipantsInRoom on ${url}`, err)
      throw new Error(`OVH Error getNumberOnlineParticipantsInRoom on ${url} : ${JSON.stringify(err)}`);
    }
  },
}
...
module.exports.getAllRooms = OVHRoomApiHelper.getAllRooms
module.exports.getNumberOnlineParticipantsInRoom = OVHRoomApiHelper.getNumberOnlineParticipantsInRoom