...
module.exports.addPublicWebAccess = async (phoneNumber, roomNumber, type="read") => {
  const url = `/telephony/${config.OVH_ROOM_ACCOUNT_NUMBER}/conference/${phoneNumber}/rooms/${roomNumber}/webAccess`
  try {
    return await ovhRooms.requestPromised("POST", url, {
      type
    })
  } catch (err) {
    console.error(`Error addPublicWebAccess on ${url}`, err)
    throw new Error(`Error addPublicWebAccess on ${url} : ${JSON.stringify(err)}`)
  }
}
