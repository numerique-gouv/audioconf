...
module.exports.anonymizeConferences = async () => {
  try {
    ...
    const updateQueries = conferencesToUpdate.map(({ id, email }) =>
      knex("conferences")
        .where("id", id)
        .update({ email: "", domain: domainFromEmail(email) })
    )

    const nbQueries = await Promise.all(updateQueries)

    console.debug("Nombre de conférences anonymisés :", nbQueries.length)

    return nbQueries.length
  } catch (err) {
    throw new Error("Erreur lors de l'anonymisation des conférences")
  }
}
