...
const PATTERN_ANONYMOUS_EMAIL = "xxxxx"

function anonymizeEmail(email) {
  if (!email) return PATTERN_ANONYMOUS_EMAIL

  const [_, domain] = email.split("@")

  return domain ? `${PATTERN_ANONYMOUS_EMAIL}@${domain}` : PATTERN_ANONYMOUS_EMAIL
}
...
module.exports.anonymizeConferences = async () => {
  try {
    const conferencesToUpdate = await knex("conferences")
      .select("id", "email")
      .whereRaw(`email not like '${PATTERN_ANONYMOUS_EMAIL}%'`)
      .whereRaw(`"expiresAt" < now() - interval '2 days'`)

    if (!conferencesToUpdate.length) {
        console.debug("Nombre de conférences anonymisés : 0")
        return
    }
    ...
    const updateQueries = conferencesToUpdate.map(({ id, email }) =>
      knex("conferences")
        .where("id", id)
        .update({ email: anonymizeEmail(email) })
    )

    Promise.all(updateQueries).then((nbQueries) => {
      console.log("Nombre de conférences anonymisés : ", nbQueries.length)
    })

  } catch (err) {
    throw new Error("Erreur lors de l'anonymisation des emails des conférences")
  }
}
