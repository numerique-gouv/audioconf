...
// fetch conferences with email but no hashedEmail
module.exports.getConferencesWithNoHashedEmails = async () =>
   knex("conferences").whereNotNull("email").whereNull("hashedEmail").select("*")


module.exports.updateHashedEmail = async ({ id, email }) =>
   knex("conferences")
    .where("id", id)
    .update({
      hashedEmail: format.hashForLogs(email),
    })
...
module.exports.fillSurveyDateConference = async (hashedEmail) => {
  try {
    ...
    console.error(`Impossible de mettre à jour la conférence ${id}`, err)
    throw new Error("Impossible de mettre à jour la conférence")
  }
}
