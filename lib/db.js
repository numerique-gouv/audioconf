...
// conferenceDayString format : YYY-MM-DD
module.exports.insertOidcRequest = async (
        state,
        nonce,
        conferenceDurationInMinutes,
        conferenceDayString,
      ...
      try {
        return await knex("oidcRequest").insert({
          state,
          nonce,
          durationInMinutes: conferenceDurationInMinutes,
          conferenceDay: conferenceDayString,
        ...
        console.error("Erreur de sauvegarde du oidcRequest", err)
        throw new Error("Erreur de sauvegarde du oidcRequest")
      }
}

module.exports.getOidcRequest = async (state) => {
  try {
      const oidcRequests = await knex("oidcRequest").select()
          .where({ state: state })
          .del()
          .returning("*")
      // Format the conferenceDays to be YYYY-MM-DD strings, rather than full Date objects
      // (the field is a table.date in knex, not a table.datetime, so it's less confusing to use a dateString.)
      oidcRequests.forEach(oidcRequest => {
          if (oidcRequest.conferenceDay) {
              const dayString = format.formatStandardDate(oidcRequest.conferenceDay)
              oidcRequest.conferenceDay = dayString
          }
     })
     return oidcRequests
  } catch (err) {
    console.error("Impossible de récupérer le oidcRequest", err)
    throw new Error("Impossible de récupérer le oidcRequest")
  }
}
...
module.exports.recordSurveySentAt = async (hashedEmail) => {
  try {
    ...
    console.error(`Error during fill survey date ${hashedEmail}`, err)
...
module.exports.insertSurveyMarks = async (surveyDaySummary) => {
  const  {markCount, averageMark, surveyDate} = surveyDaySummary
  try {
    const [row] = await knex("surveyMarks")
      .insert({
        count: markCount,
        average: averageMark,
    ...
    console.error("insertSurveyMarks error", err)
  }
}
