...
module.exports.finishAuth = async (req) => {
  ...
  try {
    ...
    if(!domain || !whitelistedDomains.includes(domain)){
      ...
      return { error: `L'adresse e-mail ${email} n'est pas autorisée à utiliser ce service. Si vous êtes agent de l'État, contactez-nous à support@audioconf.numerique.gouv.fr` }
    }
  } catch(e){
