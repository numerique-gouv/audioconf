...
module.exports.sendConfCreatedEmail = async function(toEmail, phoneNumber, pin, durationInMinutes, freeAt, confUrl, pollUrl) {
  ...
  <div class="paragraph">
    <div>Ce numéro de conférence est actif pendant ${format.formatMinutesInHours(durationInMinutes)}. Il expirera ${format.formatFrenchDateTime(freeAt)}.</div>
  </div>
  ...
  try {
    return await sendMail(
      config.MAIL_SENDER_EMAIL,
      toEmail,
      'Votre conférence téléphonique est réservée',
      html,
    )
  } catch (err) {
