...
module.exports.sendConfCreatedEmail = async function(toEmail, confPhoneNumber, pin, freeAt) {
  const html = `
  <p>Bonjour,</p>
  <p></p>
  ...
  <ul>
    <li>appelez le <strong>${format.formatFrenchPhoneNumber(confPhoneNumber)}</strong> sur votre téléphone pro ou perso</li>
    <li>tapez le code d'accès à ${config.NUM_PIN_DIGITS} chiffres : <strong>${pin}</strong></li>
  </ul>
  <p></p>
  <p>Ce numéro de conférence est actif pendant ${config.CONFERENCE_DURATION_IN_MINUTES/60} heure${ config.CONFERENCE_DURATION_IN_MINUTES >= 120 ? 's' : ''}. Il expirera ${format.formatFrenchDate(freeAt)}.</p>
