...
const sendMail = async function (fromEmail, toEmail, subject, html, replyTo) {
  const mail = {
    to: toEmail,
    ...
    replyTo: `${config.APP_NAME} <${replyTo|fromEmail}>`
  }
...
module.exports.shouldSendWebAccessMail = function(email) {
...
module.exports.sendSurveyEmail = async function (toEmail) {
  ...
  try {
    return sendMail(config.MAIL_NOREPLY_EMAIL, toEmail, `Donner votre avis sur l'audioconférence d'hier`, html, config.MAIL_SENDER_EMAIL)
  } catch (err) {
