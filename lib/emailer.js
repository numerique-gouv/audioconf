...
module.exports.sendConfCreatedEmail = async function(toEmail, phoneNumber, pin, durationInMinutes, freeAt, confUrl, pollUrl) {
  ...
  if(pollUrl) {  
    pollHtml = `<hr />
    <div class="paragraph">
      <div>Vous avez une remarque ou une suggestion ? <a title="Donner mon avis" rel="noopener noreferrer nofollow" target="_blank" href="${pollUrl}">Donnez-nous votre avis sur le service</a>.</div>
    </div>`
  }
  ...
  <div class="paragraph">
    <div>Votre conférence téléphonique du 11 janvier 2020 est réservée. Elle pourra accueillir jusqu'à 50 personnes.</div>
    <div>Pour vous y connecter : <div>
    <ul>
      <li>appelez le <strong>${format.formatFrenchPhoneNumber(phoneNumber)}</strong> (numéro non surtaxé) sur votre téléphone fixe ou mobile</li>
      <li>tapez le code d'accès à ${config.NUM_PIN_DIGITS} chiffres : <strong>${pin}</strong></li>
    </ul>
  </div>

  <div class="paragraph">
    <div>Ce numéro de conférence sera actif toute la journée du vendredi 11 janvier 2020 et expirera à minuit.</div>
  </div>
  ...
  try {
    return await sendMail(
      config.MAIL_SENDER_EMAIL,
      toEmail,
      'Votre conférence téléphonique du vendredi 11 janvier 2020 est réservée',
      html,
    )
  } catch (err) {
