...
module.exports.sendSurveyEmail = async function (toEmail) {
  ...
  <p>Bonjour,</p>
  <p></p>
  <p>Vous avez récemment utilisé le service de conférence <b>${config.APP_NAME}</b>.</p>
  ...
  try {
    return sendMail(config.MAIL_SENDER_EMAIL, toEmail, `Donner votre avis sur ${config.APP_NAME}`, html)
  } catch (err) {
